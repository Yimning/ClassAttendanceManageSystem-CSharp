...
{
    public partial class Form_stuChangepsw : Form
    {
        public Form_stuChangepsw()
        {
            InitializeComponent();
        }
            ...
            else if (textBox2.Text.Equals(textBox3.Text))
            {
                if (textBox1.Text.Trim().Equals(Form_login.str1))
                {
                    try
                    {
                        ...
                        string str = "UPDATE studentInfo SET stuPassword ='" + textBox3.Text.Trim() + "'where stuID = '" + Form_login.str.Trim() + "'";
                        myconn.Open();
                        SqlCommand xg;
                        ...
                        {                   //执行要操作的语句           
                            ...
                            MessageBox.Show("修改成功,原密码退出之后失效","提示");
                            Close();
                            myconn.Close();
                        }
                    }
